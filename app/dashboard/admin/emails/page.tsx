    ...
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500">Customize automated registration receipts, paper notifications, and reminders</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {templates.map((t) => (
          <div key={t.id} className="p-6 rounded-3xl bg-white border border-[#1E40AF]/15 shadow-sm space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-slate-700 block mb-1">Body Template</label>
              <textarea
                rows={4}
                value={t.body}
                onChange={(e) => {
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs text-slate-900 font-mono"
              />
            </div>
          </div>
        ))}
