  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight">On-Site Attendance & QR Check-In Terminal</h1>
          <p className="text-xs text-slate-400">Validate participant QR badges and record real-time venue check-ins</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
          <span className="text-[10px] text-slate-400 font-mono">Total Registrations</span>
          <span className="text-2xl font-black text-white block mt-1">14</span>
        </div>
        <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
          <span className="text-[10px] text-slate-400 font-mono">Checked In</span>
          <span className="text-2xl font-black text-emerald-400 block mt-1">5</span>
        </div>
        <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800">
          <span className="text-[10px] text-slate-400 font-mono">Not Checked In</span>
          <span className="text-2xl font-black text-amber-400 block mt-1">9</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Scan or Enter Badge Code</h3>
              <p className="text-xs text-slate-400">Enter registration code (e.g. DV-REG-1001 or QR string)</p>
            </div>
          </div>
          ...
          <form onSubmit={handleCheckIn} className="space-y-4">
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white font-mono placeholder-slate-500 focus:outline-none focus:border-accent-cyan/50"
            />
        ...
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-4 flex flex-col justify-between">
          <h3 className="text-base font-bold text-white tracking-wide border-b border-slate-800 pb-3">Latest Validated Check-In</h3>

          {lastCheckIn ? (
            <div className="p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono text-emerald-400 bg-emerald-500/20 px-2.5 py-0.5 rounded-full font-bold">
              ...
              <div>
                ...
                <p className="text-xs text-slate-300 font-mono">{lastCheckIn.user?.email}</p>
                <p className="text-xs text-slate-400 mt-1">{lastCheckIn.user?.institution}</p>
              </div>
              <div className="pt-2 border-t border-emerald-500/20 flex justify-between items-center text-xs">
                <span className="font-mono text-accent-cyan">{lastCheckIn.registrationCode}</span>
                <span className="font-bold text-white">{lastCheckIn.conference?.acronym}</span>
              </div>
            </div>
          ) : (
            <div className="text-center py-12 text-slate-500 text-xs">No check-in scanned yet</div>
          )}
