    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-3xl bg-white border border-[#1E40AF]/15 shadow-sm space-y-1">
          ...
          <span className="text-2xl font-black text-emerald-800 block mt-1">5</span>
        </div>
        <div className="p-5 rounded-3xl bg-white border border-[#1E40AF]/15 shadow-sm space-y-1">
          <span className="text-[11px] text-slate-700 font-bold font-mono uppercase tracking-wider">Not Checked In</span>
          <span className="text-2xl font-black text-amber-800 block mt-1">9</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="p-6 rounded-3xl bg-white border border-[#1E40AF]/15 shadow-sm space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-[#0D1117]">Scan or Enter Badge Code</h3>
              <p className="text-xs text-slate-600 font-medium">Enter registration code (e.g. DV-REG-1001 or QR string)</p>
            </div>
          </div>
          ...
          <form onSubmit={handleCheckIn} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-500 text-slate-950 font-extrabold text-xs tracking-wider uppercase shadow-md"
            >
        ...
        <div className="p-6 rounded-3xl bg-white border border-[#1E40AF]/15 shadow-sm space-y-4 flex flex-col justify-between">
          <h3 className="text-base font-bold text-[#0D1117] tracking-wide border-b border-slate-200 pb-3">Latest Validated Check-In</h3>

          {lastCheckIn ? (
            <div className="p-6 rounded-2xl bg-emerald-50 border border-emerald-200 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono text-emerald-800 bg-emerald-100 px-2.5 py-0.5 rounded-full font-bold uppercase border border-emerald-300">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 font-medium mt-1">{lastCheckIn.user?.institution}</p>
              </div>
              <div className="pt-2 border-t border-emerald-200 flex justify-between items-center text-xs">
                <span className="font-mono font-bold text-[#1E40AF]">{lastCheckIn.registrationCode}</span>
                <span className="font-bold text-[#0D1117]">{lastCheckIn.conference?.acronym}</span>
              </div>
            </div>
          ) : (
            <div className="text-center py-12 text-slate-600 font-bold text-xs">No check-in scanned yet</div>
          )}

          <div className="text-center text-[10px] text-slate-600 font-mono uppercase tracking-widest font-bold">
