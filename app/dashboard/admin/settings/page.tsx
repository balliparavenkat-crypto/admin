...
import { Settings, Save, Key, Mail, Shield, Check } from "lucide-react";
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight">Platform System Settings</h1>
          <p className="text-xs text-slate-400">Configure payment gateway keys, SMTP email credentials, and platform defaults</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-6">
        <div className="space-y-4">
          <h3 className="text-base font-bold text-white tracking-wide border-b border-slate-800 pb-3 flex items-center gap-2">
            <Key className="w-4 h-4 text-accent-cyan" /> Payment Gateway Integration Keys
          </h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-2 font-mono">Razorpay Key ID</label>
              <input
                type="password"
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-2 font-mono">Stripe Publishable Key</label>
              <input
                type="password"
        ...
        <div className="space-y-4 pt-4 border-t border-slate-800">
          <h3 className="text-base font-bold text-white tracking-wide border-b border-slate-800 pb-3 flex items-center gap-2">
            <Mail className="w-4 h-4 text-amber-400" /> Transactional Email SMTP
          </h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-2">SMTP Host</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-2">SMTP Port</label>
              <input
                type="text"
