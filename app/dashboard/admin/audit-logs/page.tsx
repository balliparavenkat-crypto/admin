    ...
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-700 font-medium">Complete security trail of administrator actions, registrations, and logins</p>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-white border border-[#1E40AF]/15 flex items-center gap-4 shadow-sm">
        ...
        <input
          ...
          className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-2 text-xs text-slate-900 font-semibold placeholder-slate-500 focus:outline-none focus:border-[#1E40AF]"
        />
      </div>
            ...
            <tbody className="divide-y divide-slate-200 text-slate-900 font-medium">
              {filtered.map((l) => (
                <tr key={l.id} className="hover:bg-slate-50 transition-colors">
                  <td className="p-4 font-mono text-slate-700 font-bold">{new Date(l.timestamp).toLocaleString()}</td>
                  <td className="p-4 font-bold text-[#0D1117]">{l.userEmail}</td>
                  <td className="p-4 font-mono font-black text-[#1E40AF]">{l.action}</td>
