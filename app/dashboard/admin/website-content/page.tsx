    ...
    <AdminLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500">Dynamically update public homepage sections (Hero, About, FAQs) without code deployment</p>
        </div>
        ...
        <div className="md:col-span-3 p-6 rounded-3xl bg-white border border-[#1E40AF]/15 shadow-sm space-y-6">
          ...
          <div className="space-y-4">
            {Object.keys(sections[activeSection] || {}).map((key) => (
              <div key={key}>
                <label className="text-xs font-semibold text-slate-700 block mb-1 font-mono">{key}</label>
                <textarea
                  rows={2}
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-900 focus:outline-none focus:border-[#1E40AF]"
                />
              </div>
            ))}
          </div>
